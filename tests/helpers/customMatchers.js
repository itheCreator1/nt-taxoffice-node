...
expect.extend({
  ...
  async toExistInDatabase(appointmentId) {
    ...
    try {
      const [rows] = await pool.query('SELECT id FROM appointments WHERE id = ?', [appointmentId]);
      ...
      return {
        message: () =>
          `expected appointment ${appointmentId} to exist in database, but it was not found`,
        pass: false,
      };
    } catch (error) {
      return {
  ...
  async toHaveStatusInDatabase(appointmentId, expectedStatus) {
    ...
    try {
      const [rows] = await pool.query('SELECT status FROM appointments WHERE id = ?', [
        appointmentId,
      ]);
  ...
  toMatchAppointmentSchema(received) {
    ...
    const failureMessages = failures.map(
      (f) => `${f.key}: expected ${f.expected}, got ${f.actual}`
    );
    return {
      message: () =>
