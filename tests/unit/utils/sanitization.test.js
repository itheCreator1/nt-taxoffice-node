...
describe('Sanitization Utility', () => {
  describe('escapeHtml', () => {
    test('should escape HTML special characters', () => {
      expect(sanitization.escapeHtml('<script>alert("xss")</script>')).toBe(
  ...
  describe('sanitizeAvailabilitySettings', () => {
    ...
    test('should handle boolean conversion', () => {
      ...
      expect(
        sanitization.sanitizeAvailabilitySettings({ is_working_day: false }).is_working_day
      ).toBe(false);
      expect(
        sanitization.sanitizeAvailabilitySettings({ is_working_day: true }).is_working_day
      ).toBe(true);
      expect(sanitization.sanitizeAvailabilitySettings({ is_working_day: 1 }).is_working_day).toBe(
        true
      );
      expect(sanitization.sanitizeAvailabilitySettings({ is_working_day: 0 }).is_working_day).toBe(
        false
      );
    });
  });
  ...
  describe('sanitizeSql', () => {
    test('should remove SQL comment sequences', () => {
      expect(sanitization.sanitizeSql('SELECT * FROM users -- comment')).toBe(
        'SELECT * FROM users  comment'
      );
      expect(sanitization.sanitizeSql('SELECT /* comment */ * FROM users')).toBe(
        'SELECT  comment  * FROM users'
      );
    });

    test('should remove semicolons', () => {
      expect(sanitization.sanitizeSql('SELECT * FROM users; DROP TABLE users;')).toBe(
