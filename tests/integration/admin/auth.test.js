...
describe('Admin Auth API Integration Tests', () => {
  ...
  describe('POST /api/admin/setup', () => {
    ...
    test('should reject setup if admin already exists', async () => {
      ...
      const response = await request(app).post('/api/admin/setup').send({
        username: 'admin2',
    ...
    test('should hash password before storing', async () => {
      ...
      const [rows] = await db.query('SELECT password_hash FROM admin_users WHERE username = ?', [
        'admin',
      ]);
  ...
  describe('POST /api/admin/login', () => {
    ...
    test('should update last_login timestamp on successful login', async () => {
      ...
      const [beforeRows] = await db.query('SELECT last_login FROM admin_users WHERE username = ?', [
        'admin',
      ]);
      ...
      const [afterRows] = await db.query('SELECT last_login FROM admin_users WHERE username = ?', [
        'admin',
      ]);
    ...
    test('should create session on successful login', async () => {
      ...
      const response = await agent.get('/api/admin/me').expect(200);
  ...
  describe('POST /api/admin/logout', () => {
    ...
    test('should destroy session on logout', async () => {
      ...
      const response = await agent.get('/api/admin/me').expect(401);
  ...
  describe('GET /api/admin/check-setup', () => {
    test('should return setupRequired: true when no admin exists', async () => {
      const response = await request(app).get('/api/admin/check-setup').expect(200);
