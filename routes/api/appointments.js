 ...
 */
router.post('/book', bookingLimiter, asyncHandler(async (req, res) => {
    // Sanitize input
    ...
    const available = await isSlotAvailable(
        ...
        sanitized.appointment_time
    );

    if (!available) {
        throw conflict('Αυτή η χρονική υποδοχή δεν είναι πλέον διαθέσιμη. Παρακαλώ επιλέξτε άλλη ώρα.');
    }
    ...
    try {
        ...
        if (error.message === 'SLOT_ALREADY_BOOKED') {
            throw conflict('Αυτή η χρονική υποδοχή μόλις κρατήθηκε από άλλον χρήστη. Παρακαλώ επιλέξτε άλλη ώρα.');
        }
        throw error;
    }
}));
 ...
 */
router.post('/:token/cancel', cancellationLimiter, asyncHandler(async (req, res) => {
    const { token } = req.params;
