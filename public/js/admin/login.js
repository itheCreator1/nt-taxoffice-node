  ...
  try {
    ...
    if (response.ok && data.success) {
      ...
      if (response.status === 401) {
        showMessage(data.message || 'Λάθος όνομα χρήστη ή κωδικός πρόσβασης.', 'error');
        ...
        showMessage('Απαιτείται αρχική ρύθμιση του συστήματος. Ανακατεύθυνση...', 'warning');
        setTimeout(() => {
          window.location.href = '/admin/setup.html';
        }, 2000);
      } else {
        showMessage(data.message || 'Σφάλμα κατά τη σύνδεση. Παρακαλώ δοκιμάστε ξανά.', 'error');
      }
